...
/**
 * Left column content — eyebrow + heading + body + 3 features + CTA.
 * Used standalone inside VisionMissionBlock, and by PinnedCrossfade.
 */
export const VisionMissionLeftContent: React.FC<Props> = ({
  eyebrow,
  eyebrowIcon,
  heading,
  body,
...
/**
 * Right column content — mascot + vision/mission cards.
 * Renders TWO siblings designed to live inside a `grid grid-cols-7` parent:
 *   - mascot column: `col-span-4`
 *   - cards column:  `col-span-3` (stacked vertically)
 */
export const VisionMissionRightContent: React.FC<Props> = ({ mascot, vision, mission }) => {
  return (
    <>
      {/* Mascot col */}
 ...
 * with a following CoreValues block). Includes background, container, and
 * grid layout. When paired, PinnedCrossfade renders Left/Right sub-components
 * directly and supplies its own background.
 */
export const VisionMissionBlock: React.FC<Props> = (props) => {
  return (
